...
As depicted in *Figure #* and described in [13], each *FN* node now contains information about its peer subset, which is, in [13], denoted as a node's ***partial view***. *Partial view* is a list of ***node descriptors***, where each descriptor contains a node's ***IP address*** and a ***hop count***. *Hop count* represents the distance between the two corresponding nodes expressed in the number of network hops between them.

#### Concluding the idea
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQyODQ1MjE3MiwtMTY4NjM4MzQzNSw4Mz

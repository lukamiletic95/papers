...
When a *FN* node receives a transaction, it stores it inside its own Mempool. In addition, the transaction is gossiped over the network to all of the peers of that particular *FN* node. Then the peers re-gossip the received transaction and so forth.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIyNDM2MDMwMywxNzA4MDUwNTksLTQ0MD
